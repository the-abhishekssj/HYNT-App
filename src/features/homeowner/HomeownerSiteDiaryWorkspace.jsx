  ...
  return (
    <button type="button" onClick={onClick} className="flex min-h-28 flex-col justify-between rounded-[18px] border border-[#dce7df] p-3 text-left">
      ...
      <div>
        ...
        <p className="typo-caption mt-1 text-[#777]">{meta}</p>
      </div>
    </button>
  )
}
...
function FilterChip({ label, selected, onClick }) {
  return (
    <button type="button" onClick={onClick} className={`typo-caption rounded-full px-3 py-2 ${selected ? 'bg-black text-white' : 'border border-[#d8e2db] text-[#6f6f6f]'}`}>
      {label}
    </button>
  )
}
    ...
    <button type="button" onClick={() => onOpen(entry.id)} className="w-full border-b border-[#e5e5e5] py-4 text-left last:border-b-0">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="typo-meta mt-1 text-[#777]">{formatDiaryDateLabel(entry.createdAt)} / {formatDiaryTimeLabel(entry.createdAt)}</p>
        ...
        <div className="shrink-0 text-right">
          ...
          {entry.clientReviewedAt ? <p className="typo-caption mt-2 text-[#267449]">Reviewed</p> : null}
        </div>
      </div>
    </button>
  )
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className="typo-meta mt-1 text-[#777]">{formatDiaryDateLabel(reference.createdAt)}</p>
            </div>
...
function ProgressPhotoGrid({ entries, onOpen }) {
  ...
  if (!photoItems.length) {
    return <p className="typo-body px-1 py-4 text-[#6f6f6f]">No progress photos shared yet.</p>
  }
    ...
    <div className="grid grid-cols-3 gap-2">
      {photoItems.map((item) => (
        ...
        >
          ...
          <span className="typo-status-mini absolute bottom-1.5 left-1.5 rounded-full bg-[rgba(0,0,0,0.58)] px-2 py-1 text-white">
        ...
        <section className="ui-page-summary px-4 py-5">
          <div className="flex items-center justify-between gap-3">
            ...
            <button
              ...
              className={`typo-body-strong h-10 rounded-[16px] px-4 ${entry.clientReviewedAt ? 'bg-[#eaf8ef] text-[#267449]' : 'border border-[#d8e2db] text-[#173324]'}`}
            >
              {entry.clientReviewedAt ? 'Reviewed' : 'Mark reviewed'}
            </button>
          </div>
          <p className="typo-body mt-4 text-black">{entry.note || 'Photo update shared from site.'}</p>
            ...
            {entryComments.length ? (
              entryComments.map((comment, index) => (
                <div key={comment.id || `${entry.id}-comment-${index}`} className="border-b border-[#e5e5e5] py-4 last:border-b-0">
                  <div className="flex items-center justify-between gap-3">
                    <p className="typo-caption uppercase text-[#7b7b7b]">You asked</p>
                    <p className="typo-meta text-[#8a8a8a]">{formatDiaryDateLabel(comment.createdAt)}</p>
                  </div>
                  <p className="typo-body mt-1 text-black">{comment.body || comment}</p>
                  {comment.designerReply ? <p className="typo-caption mt-2 border-l-2 border-[#70b58d] pl-2 text-[#267449]">Designer replied: {comment.designerReply}</p> : null}
                </div>
              ))
            ) : (
              <div className="py-4">
                <p className="typo-body text-[#6f6f6f]">Ask about workmanship, progress, or what happens next on site.</p>
              </div>
            )}
          </div>
        ...
        <div className="ui-screen-content">
          ...
          {activeView === 'home' ? (
            <>
              <section className="border-b border-[#e5e5e5] py-5">
                <div className="mb-4 flex items-center justify-between">
                  <p className="typo-section-title text-black">Browse updates</p>
                  <span className="typo-caption text-[#777]">From your project team</span>
                </div>
                <div className="grid grid-cols-2 gap-3">
              ...
              <section className="py-5">
                <div className="mb-3 flex items-center justify-between">
                  <p className="typo-section-title text-black">Latest updates</p>
                  <button type="button" onClick={() => setActiveView('daily-log')} className="flex items-center gap-1 typo-caption text-[#777]">View logs <CaretRight size={14} /></button>
                </div>
                <div className="border-y border-[#e5e5e5]">
            ...
            <>
              <section className="border-b border-[#e5e5e5] py-5">
                <div className="mb-4 flex items-center justify-between">
                  <p className="typo-section-title text-black">Share inspiration</p>
              ...
              <section className="py-5">
                <div className="mb-4 flex items-center justify-between">
                  <p className="typo-section-title text-black">Shared references</p>
            ...
            <section className="py-5">
              <div className="mb-4 flex items-center justify-between">
                ...
                <span className="typo-caption text-[#7b7b7b]">{filteredPhotoCount} shared</span>
              </div>
            ...
            <>
              <section className="border-b border-[#e5e5e5] py-5">
                <div className="mb-4 flex items-center justify-between">
                  <p className="typo-section-title text-black">{SITE_DIARY_BUCKETS.find((bucket) => bucket.id === activeView)?.label || 'Updates'}</p>
                ...
                <div className="border-y border-[#e5e5e5]">
                  {filteredEntries.length ? filteredEntries.map((entry) => (
                    ...
                    <p className="typo-body px-1 py-4 text-[#6f6f6f]">{SITE_DIARY_BUCKETS.find((bucket) => bucket.id === activeView)?.empty}</p>
                  )}
                </div>
              </section>
              ...
              <section className="py-5">
                <div className="mb-4 flex items-center justify-between">
                  ...
                  <span className="typo-caption text-[#7b7b7b]">{siteDiaryIssues.length} tracked</span>
                </div>
                <div className="border-y border-[#e5e5e5]">
