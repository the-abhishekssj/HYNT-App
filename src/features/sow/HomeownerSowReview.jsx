...
function HomeownerSowReview({ onBack }) {
  ...
  const renderReview = () => (
    <section className="mx-auto w-full max-w-[390px] pb-[132px] pt-[56px]">
      ...
      <div className="px-4 py-5">
        <section className="pb-5">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="type-section-title text-[#102418]">{project.projectName}</p>
              <p className="type-body mt-1 text-[#5f7467]">Designer: {project.designerName} | {project.location}</p>
            </div>
            <span className="type-caption rounded-full bg-[#e7f5ec] px-3 py-1 uppercase text-[#267449]">Review</span>
          </div>
          ...
          <div className="mt-4 grid grid-cols-3 gap-2">
            <div className="flex min-h-[60px] flex-col items-center justify-center rounded-xl border border-[#dbe6df] bg-[#f4fbf7] px-2.5 py-2.5 text-center">
              ...
              <p className="type-card-title mt-1 text-[#102418]">{project.projectType}</p>
            </div>
            <div className="flex min-h-[60px] flex-col items-center justify-center rounded-xl border border-[#dce7f3] bg-[#f4f8ff] px-2.5 py-2.5 text-center">
              ...
              <p className="type-card-title mt-1 text-[#102418]">INR {project.budgetLabel}</p>
            </div>
            <div className="flex min-h-[60px] flex-col items-center justify-center rounded-xl border border-[#efe2c8] bg-[#fff9ef] px-2.5 py-2.5 text-center">
              ...
              <p className="type-card-title mt-1 text-[#102418]">{project.handoverMonth}</p>
            </div>
          </div>
        </section>
        ...
        <section className="space-y-3 py-5">
          ...
          <ClientSection index="2" title="Scope - room wise" open={openSections.scope} onToggle={() => toggleSection('scope')}>
            ...
            <button type="button" onClick={() => setView('summary')} className="type-label mt-3 flex items-center gap-2 rounded-xl border border-[#e0e0e0] bg-[#fbfbfb] px-3 py-2 text-black">
              <NotePencil size={14} />
              Add a scope remark
            </button>
          </ClientSection>
          ...
          <ClientSection index="4" title="Budget estimate" open={openSections.budget} onToggle={() => toggleSection('budget')}>
            <div className="space-y-3">
              ...
              <button type="button" onClick={() => setView('summary')} className="type-label flex items-center gap-2 rounded-xl border border-[#e0e0e0] bg-[#fbfbfb] px-3 py-2 text-black">
                <NotePencil size={14} />
                Add a budget remark
              </button>
            </div>
      ...
      <div className="fixed bottom-0 left-1/2 z-[95] w-full max-w-[390px] -translate-x-1/2 border-t border-[#e0e0e0] bg-white px-4 pb-5 pt-3 shadow-[0_-8px_24px_rgba(0,0,0,0.08)]">
        <button type="button" onClick={() => setView(hasRemarks ? 'waiting' : 'otp')} className="type-body-strong h-11 w-full rounded-full bg-black text-white">
    ...
    <section className="mx-auto w-full max-w-[390px] pb-[132px] pt-[56px]">
      ...
      <div className="space-y-3 px-4 py-5">
        <article className="rounded-2xl border border-[#dbe6df] bg-white p-4">
          <p className="type-label uppercase text-[#5f7467]">Scope feedback</p>
          <textarea
            value={remarks.scope}
            ...
            className="type-body mt-3 min-h-24 w-full rounded-2xl border border-[#dbe6df] bg-[#f7fbf8] px-4 py-3 text-black outline-none"
          />
        </article>
        <article className="rounded-2xl border border-[#dbe6df] bg-white p-4">
          <p className="type-label uppercase text-[#5f7467]">Budget feedback</p>
          <textarea
    ...
    <section className="mx-auto w-full max-w-[390px] pb-[132px] pt-[56px]">
      ...
      <div className="px-4 py-5">
        <article className="rounded-2xl border border-[#dbe6df] bg-[linear-gradient(180deg,#f4fbf7_0%,#ffffff_100%)] p-5 text-center">
          ...
          <p className="type-body mt-2 text-[#5f7467]">Your designer has been asked to revise the SOW. Review the revised version below when it is ready.</p>
        </article>
      </div>

      <div className="fixed bottom-0 left-1/2 z-[95] w-full max-w-[390px] -translate-x-1/2 border-t border-[#e0e0e0] bg-white px-4 pb-5 pt-3 shadow-[0_-8px_24px_rgba(0,0,0,0.08)]">
        <button type="button" onClick={() => setView('revised')} className="type-body-strong h-11 w-full rounded-full bg-black text-white">
          Review revised SOW
        </button>
      </div>
    </section>
      ...
      <div className="space-y-3 px-4 py-5">
        <article className="rounded-2xl border border-[#dbe6df] bg-white p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="type-card-title text-black">Your remarks were actioned.</p>
              <p className="type-body mt-1 text-[#5f7467]">Kitchen scope now includes granite countertop and chimney provision. Budget rationale remains documented for the original scope value.</p>
            </div>
          </div>
        </article>

        <article className="rounded-2xl border border-[#dbe6df] bg-white p-4">
          <p className="type-label uppercase text-[#5f7467]">Updated kitchen scope</p>
          <p className="type-body mt-2 text-black">Modular L-shape, granite countertop, soft-close shutters, chimney provision included</p>
        </article>

        <article className="rounded-2xl border border-[#dbe6df] bg-white p-4">
          <p className="type-label uppercase text-[#5f7467]">Budget note from designer</p>
          <p className="type-body mt-2 text-black">The 18L estimate protects execution quality and keeps the selected scope intact after the finish upgrade.</p>
        </article>
      </div>
  ...
  const renderOtp = () => (
    <section className="mx-auto w-full max-w-[390px] pb-[132px] pt-[56px]">
      ...
      <div className="fixed bottom-0 left-1/2 z-[95] w-full max-w-[390px] -translate-x-1/2 border-t border-[#e0e0e0] bg-white px-4 pb-5 pt-3 shadow-[0_-8px_24px_rgba(0,0,0,0.08)]">
        <button type="button" onClick={() => setView('executed')} className="type-body-strong h-11 w-full rounded-full bg-black text-white">
          Verify & sign
        </button>
      </div>
    </section>
    ...
    <section className="mx-auto w-full max-w-[390px] pb-[132px] pt-[56px]">
      ...
      <div className="space-y-3 px-4 py-5">
        <article className="rounded-2xl border border-[#dbe6df] bg-[linear-gradient(180deg,#f4fbf7_0%,#ffffff_100%)] p-5 text-center">
          ...
          <p className="type-body mt-2 text-[#5f7467]">Your project is officially confirmed. The executed Scope of Work is now ready for download and handoff.</p>
        </article>
        ...
        <div className="grid grid-cols-2 gap-2">
          <article className="rounded-xl border border-[#e2e2e2] bg-[#fbfbfb] p-3">
            ...
            <p className="type-caption mt-1 text-[#6f6f6f]">Signed | 24 May</p>
          </article>
          <article className="rounded-xl border border-[#e2e2e2] bg-[#fbfbfb] p-3">
      ...
      <div className="fixed bottom-0 left-1/2 z-[95] w-full max-w-[390px] -translate-x-1/2 border-t border-[#e0e0e0] bg-white px-4 pb-5 pt-3 shadow-[0_-8px_24px_rgba(0,0,0,0.08)]">
        <button type="button" className="type-body-strong h-11 w-full rounded-full bg-black text-white">
