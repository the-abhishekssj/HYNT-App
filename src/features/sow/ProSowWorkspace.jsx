    ...
    <article className="overflow-hidden rounded-2xl border border-[#dbe6df] bg-white shadow-[0_10px_26px_rgba(24,40,31,0.04)]">
      <button type="button" onClick={onToggle} className="flex w-full items-center justify-between pl-3 pr-4 py-3.5 text-left">
        ...
        <span className="flex items-center gap-2">
          {badge ? <span className="rounded-full bg-[#e7f5ec] px-2 py-1 text-[10px] font-bold uppercase tracking-[0.08em] text-[#267449]">{badge}</span> : null}
    ...
    <header className="fixed left-1/2 top-0 z-[90] w-full max-w-[390px] -translate-x-1/2 border-b border-[#dfe8e2] bg-[rgba(251,255,252,0.88)] backdrop-blur-[16px]">
      <div className="px-4 py-3">
        <div className="flex items-center justify-between py-1">
          ...
          {actionLabel ? (
            <button type="button" onClick={onAction} className="rounded-full border border-[#dbe6df] bg-white px-3 py-2 text-[12px] font-bold text-black">
              {actionLabel}
            </button>
          ) : (
    ...
    <section className="mx-auto w-full max-w-[390px] pb-[132px] pt-[56px]">
      ...
      <div className="px-4 py-5">
        <section className="pb-5">
          <p className="text-[10px] font-medium uppercase tracking-[0.12em] text-[#6e907d]">Scope of work</p>
          <h1 className="mt-2 text-[24px] font-extrabold leading-7 text-black">Start with a template that already fits the project.</h1>
          <p className="mt-2 text-[14px] font-medium leading-5 text-[#6f6f6f]">No separate doc-builder look here. This stays inside the same project workspace language.</p>
        </section>
      ...
      <div className="px-4 py-5">
        <section className="pb-5">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[20px] font-extrabold leading-7 tracking-[-0.02em] text-[#102418]">{document.projectName}</p>
              <p className="mt-1 text-[14px] font-medium leading-5 text-[#5f7467]">{document.clientName} | {document.location}</p>
            </div>
            <span className="rounded-full bg-[#e7f5ec] px-3 py-1 text-[10px] font-bold uppercase tracking-[0.08em] text-[#267449]">{statusLabel}</span>
          </div>
          ...
          <div className="mt-4 grid grid-cols-3 gap-2">
            <div className="flex min-h-[60px] flex-col items-center justify-center rounded-xl border border-[#dce7f3] bg-[#f4f8ff] px-2.5 py-2.5 text-center">
              ...
              <p className="mt-1 text-[14px] font-extrabold leading-5 text-[#102418]">INR {document.totalValueLabel}</p>
            </div>
            <div className="flex min-h-[60px] flex-col items-center justify-center rounded-xl border border-[#dbe6df] bg-[#f4fbf7] px-2.5 py-2.5 text-center">
              <p className="text-[10px] font-bold leading-[14px] text-[#73867c]">Duration</p>
              <p className="mt-1 text-[14px] font-extrabold leading-5 text-[#102418]">{document.durationLabel}</p>
            </div>
            <div className="flex min-h-[60px] flex-col items-center justify-center rounded-xl border border-[#efe2c8] bg-[#fff9ef] px-2.5 py-2.5 text-center">
              ...
              <p className="mt-1 text-[14px] font-extrabold leading-5 text-[#102418]">{document.handoverMonth}</p>
            </div>
          </div>
        </section>
        ...
        <section className="space-y-3 py-5">
          ...
          <SOWSection index="5" title="Budget estimate" open={openSections.budget} onToggle={() => toggleSection('budget')} badge={actionedRemarks.includes('budget-estimate') ? 'Actioned' : undefined}>
            <div className="space-y-3 text-[14px] leading-5">
              ...
              <div className="flex items-center justify-between">
                <span className="font-semibold text-[#5f7467]">Structure</span>
                <span className="font-bold text-black">{document.paymentStructure}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="font-semibold text-[#5f7467]">GST</span>
              ...
              <article className="rounded-2xl border border-[#e2e2e2] bg-white p-3">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <p className="mt-2 text-[14px] font-bold text-black">Riya Desai</p>
                  </div>
                ...
                <p className="mt-4 text-[12px] font-medium leading-5 text-[#5f7467]">{designerSigned ? 'Verified and signed from designer side.' : 'Awaiting OTP verification from the designer.'}</p>
              </article>
              <article className="rounded-2xl border border-[#e2e2e2] bg-white p-3">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-[0.08em] text-[#7b7b7b]">Client</p>
                    <p className="mt-2 text-[14px] font-bold text-black">{document.clientName}</p>
                  </div>
                ...
                <p className="mt-4 text-[12px] font-medium leading-5 text-[#5f7467]">{clientSigned ? 'Client has already approved this SOW.' : 'Client signature will appear after review and acceptance.'}</p>
              </article>
            </div>
          </SOWSection>
        </section>
    ...
    <section className="mx-auto w-full max-w-[390px] pb-[132px] pt-[56px]">
      ...
      <div className="px-4 py-5">
        <article className="rounded-2xl border border-[#dbe6df] bg-[#f4fbf7] p-4">
          <p className="text-[16px] font-bold leading-6 text-black">Priya has reviewed this SOW.</p>
          <p className="mt-1 text-[14px] font-medium leading-6 text-[#5f7467]">Update or retain each item, then send the revised draft back through the same project flow.</p>
        </article>

        <div className="mt-4 space-y-3">
          <article className="rounded-2xl border border-[#dbe6df] bg-white p-4">
            <p className="text-[12px] font-bold uppercase tracking-[0.08em] text-[#5f7467]">{remarksById['kitchen-scope'].title}</p>
            <p className="mt-3 text-[14px] leading-6 text-black">{remarksById['kitchen-scope'].remark}</p>
            <div className="mt-3 rounded-xl border border-[#dce7f3] bg-[#f4f8ff] p-3">
              <p className="text-[10px] font-bold uppercase tracking-[0.08em] text-[#6d82a2]">Proposed update</p>
              <p className="mt-2 text-[14px] leading-6 text-black">{remarksById['kitchen-scope'].proposedScope}</p>
            </div>
          ...
          <article className="rounded-2xl border border-[#dbe6df] bg-white p-4">
            <p className="text-[12px] font-bold uppercase tracking-[0.08em] text-[#5f7467]">{remarksById['budget-estimate'].title}</p>
            <p className="mt-3 text-[14px] leading-6 text-black">{remarksById['budget-estimate'].remark}</p>
            <div className="mt-3 rounded-xl border border-[#efe2c8] bg-[#fff9ef] p-3">
              <p className="text-[10px] font-bold uppercase tracking-[0.08em] text-[#9f8350]">Response note</p>
              <p className="mt-2 text-[14px] leading-6 text-black">{remarksById['budget-estimate'].rejectionReason}</p>
            </div>
            <div className="mt-3 grid grid-cols-2 gap-2">
    ...
    <section className="mx-auto w-full max-w-[390px] pb-[132px] pt-[56px]">
      ...
      <div className="px-4 py-5">
        <article className="rounded-2xl border border-[#dbe6df] bg-[linear-gradient(180deg,#f4fbf7_0%,#ffffff_100%)] p-5 text-center">
          ...
          <p className="mt-2 text-[14px] leading-6 text-[#5f7467]">OTP sent to the registered mobile number. Confirm to sign the Scope of Work as the designer.</p>
        ...
        {amendments.map((amendment) => (
          <article key={amendment.id} className="rounded-2xl border border-[#dbe6df] bg-white p-4">
            <div className="flex items-center justify-between gap-3">
              <p className="text-[14px] font-bold text-black">{amendment.title}</p>
            ...
            <div className="mt-3 rounded-xl border border-[#dce7f3] bg-[#f8fbff] p-3">
              <p className="text-[10px] font-bold uppercase tracking-[0.08em] text-[#6d82a2]">From</p>
              <p className="mt-1 text-[12px] text-[#808080] line-through">{amendment.oldValue}</p>
              <p className="mt-3 text-[10px] font-bold uppercase tracking-[0.08em] text-[#6d82a2]">To</p>
              <p className="mt-1 text-[12px] font-semibold text-black">{amendment.newValue}</p>
              <p className="mt-3 text-[14px] leading-6 text-[#5f7467]">{amendment.reason}</p>
            </div>
          </article>
        ))}
      </div>
