      ...
      <section className="pb-5">
        ...
        <div className="grid grid-cols-2 gap-2">
          {tools.map((tool) => {
            ...
            return (
              <article key={tool.title} className="rounded-2xl border border-[#e1e1e1] bg-[#fbfbfb] p-3">
      ...
      <section className="py-5">
        ...
        <div className="mt-4 space-y-2">
          {buildQueue.map(([label, status]) => (
            <article key={label} className="flex items-center justify-between rounded-2xl border border-[#e1e1e1] bg-white px-4 py-3">
              <div>
                <p className="text-[14px] font-bold leading-5 text-black">{label}</p>
                <p className="mt-0.5 text-[12px] font-medium leading-[18px] text-[#808080]">Native HYNT screen treatment</p>
              </div>
              <span className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.08em] ${status === 'Live now' ? 'bg-[#eaf9f1] text-[#2a9a64]' : status === 'Queued' ? 'bg-[#f2f2f2] text-[#777777]' : 'bg-[#fff7e8] text-[#a46a10]'}`}>
                {status}
              </span>
            </article>
          ))}
